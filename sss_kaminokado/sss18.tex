...
\begin{figure}
    \vspace{5mm}
    \centerline{\epsfig{file=aigamo_robot_model.eps,width=50mm}}
    \caption{Robot model}
\end{figure}
\section{カルマンフィルタの説明}
この章では提案するカルマンフィルタのモデルを説明をする．
本研究ではアイガモロボットを対象としており，このロボットは２次元平面上を移動する．
ロボットは図に示すような差動二輪車両モデルである．
カルマンフィルタの予測フェーズ
\begin{align}
...
\subsection{Result}
\begin{figure}
\centerline{\epsfig{file=process_noise_S.eps,width=100mm}}
\caption{Large Process noise}
\centerline{\epsfig{file=process_noise_M.eps,width=100mm}}
\caption{Middle Process noise}
\centerline{\epsfig{file=process_noise_L.eps,width=100mm}}
